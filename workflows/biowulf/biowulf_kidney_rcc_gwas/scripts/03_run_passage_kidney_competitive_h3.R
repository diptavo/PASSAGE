...
message("Competitive H3 kidney sample=", sample_id, " B=", B)
fit_path <- file.path(root, "results", "passage_kidney_h1_h3", sample_id, "passage_kidney_h1_h3_result.rds")
...
score_pathway <- function(ii, statistic) {
  ...
  if (inherits(res, "error")) {
    return(data.frame(
      sample = sample_id, cancer = fit$cancer, disease = fit$disease,
      spatial_sample = fit$spatial_sample, reference = fit$reference,
...
out_dir <- file.path(root, "results", "passage_kidney_competitive_h3", sample_id)
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
write.csv(tbl, file.path(out_dir, "passage_kidney_competitive_h3.csv"), row.names = FALSE)
saveRDS(list(sample = sample_id, cancer = fit$cancer, disease = fit$disease,
             spatial_sample = fit$spatial_sample, reference = fit$reference,
             table = tbl, B = B, gene_bins = gene_bins),
        file.path(out_dir, "passage_kidney_competitive_h3.rds"))
