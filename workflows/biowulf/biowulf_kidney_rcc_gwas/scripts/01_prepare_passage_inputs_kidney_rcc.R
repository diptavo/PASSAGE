...
read_10x <- function(matrix_dir) {
  mtx <- find_one(matrix_dir, "^matrix[.]mtx([.]gz)?$")
  features <- find_one(matrix_dir, "^(features|genes)[.]tsv([.]gz)?$")
  barcodes <- find_one(matrix_dir, "^barcodes[.]tsv([.]gz)?$")
  if (any(is.na(c(mtx, features, barcodes)))) {
    stop("Missing 10x matrix/features/barcodes under ", matrix_dir)
  }
...
read_positions <- function(spatial_dir) {
  pos <- find_one(spatial_dir, "^tissue_positions(_list)?[.]csv$")
  if (is.na(pos)) stop("Missing tissue_positions*.csv under ", spatial_dir)
...
read_annotation <- function(path, barcodes) {
  if (is.na(path) || !nzchar(path) || !file.exists(path)) return(data.frame(barcode = barcodes))
  ann <- tryCatch(read.csv(path, stringsAsFactors = FALSE, check.names = FALSE), error = function(e) NULL)
  if (is.null(ann) || !nrow(ann)) return(data.frame(barcode = barcodes))
  bc_col <- intersect(c("barcode", "Barcode", "barcodes", "spot", "Spot"), colnames(ann))[1L]
  if (is.na(bc_col)) return(data.frame(barcode = barcodes))
  colnames(ann)[colnames(ann) == bc_col] <- "barcode"
  ann[match(barcodes, ann$barcode), , drop = FALSE]
}

make_input <- function(row, reference_name) {
  tenx <- read_10x(row$matrix_dir)
  ...
  ref <- marker_refs[marker_refs$cancer == "kidney" & marker_refs$reference == reference_name, , drop = FALSE]
  if (!nrow(ref)) stop("No marker reference ", reference_name)
  X_tech <- data.frame(
    intercept = 1,
  ...
  ann <- read_annotation(row$annotation_file, common)
  spot_data <- cbind(pos, ann[, setdiff(colnames(ann), "barcode"), drop = FALSE])
  sample_id <- paste("kidney", row$spatial_sample, reference_name, sep = "__")
...
refs <- c("tumor_tme_tls_broad", "normal_kidney_broad")
manifest <- list()
for (ii in seq_len(nrow(spatial_manifest))) {
  for (rr in refs) {
    message("Preparing ", spatial_manifest$spatial_sample[[ii]], " with reference ", rr)
...
message("Prepared ", nrow(manifest), " kidney PASSAGE input files in ", out_dir)
