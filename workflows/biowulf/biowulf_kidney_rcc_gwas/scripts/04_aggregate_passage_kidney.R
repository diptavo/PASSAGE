...
out_dir <- file.path(root, "results", "passage_kidney_summary")
...
h_files <- file.path(root, "results", "passage_kidney_h1_h3", manifest$sample,
                     "passage_kidney_h1_h3_pathways.csv")
c_files <- file.path(root, "results", "passage_kidney_competitive_h3", manifest$sample,
                     "passage_kidney_competitive_h3.csv")
h <- read_many(h_files)
c <- read_many(c_files)

if (nrow(h)) {
  h_summary <- do.call(rbind, lapply(split(h, split_key(h, c("reference", "pathway"))), function(z) {
    data.frame(
      ...
      median_R2_cca = stats::median(z$R2_cca, na.rm = TRUE),
      median_PSVS_range = stats::median(z$PSVS_range, na.rm = TRUE),
      median_mean_propSV = stats::median(z$mean_propSV, na.rm = TRUE),
      stringsAsFactors = FALSE
    )
  }))
  h_summary$fisher_fdr_H1 <- ave(h_summary$fisher_p_H1, h_summary$reference,
                                 FUN = function(p) stats::p.adjust(p, method = "BH"))
  h_summary$fisher_fdr_H3 <- ave(h_summary$fisher_p_H3, h_summary$reference,
                                 FUN = function(p) stats::p.adjust(p, method = "BH"))
  h_summary <- h_summary[order(h_summary$reference, h_summary$fisher_p_H3), , drop = FALSE]
} else {
...
if (nrow(c)) {
  ...
  c$competitive_score_fdr_global <- ave(c$competitive_score_p, c$reference, c$statistic,
                                        FUN = function(p) stats::p.adjust(p, method = "BH"))
  c$competitive_score_empirical_leave_sample_fdr <- ave(
    c$competitive_score_empirical_leave_sample_p, c$reference, c$statistic,
    FUN = function(p) stats::p.adjust(p, method = "BH")
  )
  c_summary <- do.call(rbind, lapply(split(c, split_key(c, c("reference", "statistic", "pathway"))), function(z) {
    data.frame(
      reference = z$reference[[1L]],
  ...
  c_summary$fisher_fdr <- ave(c_summary$fisher_p, c_summary$reference, c_summary$statistic,
                              FUN = function(p) stats::p.adjust(p, method = "BH"))
  c_summary <- c_summary[order(c_summary$reference, c_summary$statistic, c_summary$fisher_p), , drop = FALSE]
} else {
...
md <- c(
  ...
  "",
  "Primary H3 covariates are technical covariates plus broad kidney/TME/TLS marker-derived scores.",
  "Competitive H3 uses matched-module Monte Carlo p-values for score_z and score_robust_z; cEPSV and PC1 coherence are descriptors.",
  "",
  "## Top Competitive H3",
  ""
)
if (nrow(c_summary)) {
  top <- head(c_summary[, c("reference", "statistic", "pathway", "n_raw_p05",
                            "n_raw_fdr05", "n_empirical_fdr05", "median_p",
                            "fisher_p", "fisher_fdr", "median_cEPSV",
                            "median_coherence_pc1")], 30)
