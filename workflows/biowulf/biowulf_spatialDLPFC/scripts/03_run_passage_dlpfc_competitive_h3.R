...
message("Competitive H3 sample=", sample_id, " B=", B)
fit_path <- file.path(root, "results", "passage_dlpfc_h1_h3", sample_id, "passage_dlpfc_h1_h3_result.rds")
...
score_pathway <- function(ii, statistic) {
  ...
  res <- tryCatch(
    passage_conditional_competitive_test(
      ...
      statistic = statistic,
      B = B,
      seed = 300000L + 10000L * task_id + 100L * ii + match(statistic, c("score_z", "score_robust_z")),
      sampler = "module",
  ...
  if (inherits(res, "error")) {
    return(data.frame(
      sample = sample_id, subject = fit$subject, section = fit$section,
      pathway = pname, pathway_size = length(P), statistic = statistic,
      competitive_score = NA_real_, competitive_score_p = NA_real_,
...
jobs <- expand.grid(ii = seq_along(pathways), statistic = c("score_z", "score_robust_z"),
                    stringsAsFactors = FALSE)
rows <- parallel::mclapply(seq_len(nrow(jobs)), function(jj) {
  score_pathway(jobs$ii[[jj]], jobs$statistic[[jj]])
}, mc.cores = cores, mc.preschedule = FALSE)
tbl <- do.call(rbind, rows)
tbl$competitive_score_fdr <- ave(tbl$competitive_score_p, tbl$statistic, FUN = function(p) stats::p.adjust(p, method = "BH"))
tbl <- tbl[order(tbl$statistic, tbl$competitive_score_p), , drop = FALSE]

out_dir <- file.path(root, "results", "passage_dlpfc_competitive_h3", sample_id)
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
write.csv(tbl, file.path(out_dir, "passage_dlpfc_competitive_h3.csv"), row.names = FALSE)
saveRDS(list(sample = sample_id, table = tbl, B = B, gene_bins = gene_bins),
        file.path(out_dir, "passage_dlpfc_competitive_h3.rds"))
