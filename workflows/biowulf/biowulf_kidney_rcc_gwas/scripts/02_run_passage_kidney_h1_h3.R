...
message("PASSAGE kidney H1/H3 sample=", sample_id, " task=", task_id)
...
hallmark_pathways <- local({
  ...
  if (!file.exists(pathway_file)) stop("Missing cached Hallmark pathway file: ", pathway_file)
  lapply(readRDS(pathway_file), function(g) unique(toupper(g)))
})
...
if (!length(pathways)) stop("No Hallmark pathways overlap sample ", sample_id)
...
engine <- passage_fit_engine_pca(
  Y = Y, coords = coords, X = X_tech, K = 6L, m = 20L,
  range_grid = range_grid, kernel = "matern32", verbose = TRUE
)
...
score_one <- function(ii) {
  ...
  common <- list(
    engine = engine, Y = Y, pathway = genes,
    weight_schemes = c("equal", "var", "range"),
    gene_names = colnames(Y), calibration = "permutation",
    n_perm = n_perm, run_burden = TRUE, run_spasset = FALSE
  )
  ...
  data.frame(
    sample = sample_id,
    cancer = obj$cancer,
    disease = if (!is.null(obj$disease)) obj$disease else "renal_cell_carcinoma",
...
out_dir <- file.path(root, "results", "passage_kidney_h1_h3", sample_id)
...
fit <- list(
  ...
  n_perm = n_perm, K = 6L, m = 20L, range_grid = range_grid,
...
saveRDS(fit, file.path(out_dir, "passage_kidney_h1_h3_result.rds"))
write.csv(tbl, file.path(out_dir, "passage_kidney_h1_h3_pathways.csv"), row.names = FALSE)
writeLines(c(
  paste0("# PASSAGE kidney H1/H3: ", sample_id),
  "",
