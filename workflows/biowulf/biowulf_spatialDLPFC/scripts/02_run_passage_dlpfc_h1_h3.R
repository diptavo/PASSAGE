...
if (task_id < 1L || task_id > nrow(manifest)) {
  stop("task_id out of range: ", task_id, " for manifest rows=", nrow(manifest))
}
sample_row <- manifest[task_id, , drop = FALSE]
sample_id <- sample_row$sample[[1L]]
message("PASSAGE H1/H3 sample=", sample_id, " task=", task_id)
...
message("Fitting engine: spots=", nrow(Y), " hallmark_genes=", ncol(Y), " pathways=", length(pathways))
...
score_one <- function(ii) {
  ...
  data.frame(
    sample = sample_id,
    subject = sub("_.*$", "", sample_id),
    section = sub("^[^_]+_", "", sample_id),
    pathway = pname,
...
out_dir <- file.path(root, "results", "passage_dlpfc_h1_h3", sample_id)
...
saveRDS(fit, file.path(out_dir, "passage_dlpfc_h1_h3_result.rds"))
write.csv(tbl, file.path(out_dir, "passage_dlpfc_h1_h3_pathways.csv"), row.names = FALSE)
writeLines(c(
  paste0("# PASSAGE H1/H3: ", sample_id),
  "",
