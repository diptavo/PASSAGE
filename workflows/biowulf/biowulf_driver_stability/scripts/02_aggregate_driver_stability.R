...
if (nrow(summ)) {
  cohort_stat <- do.call(rbind, lapply(split(summ, list(summ$cohort, summ$statistic), drop = TRUE), function(z) {
    data.frame(
      cohort = z$cohort[[1L]],
  ...
  cohort_stat <- cohort_stat[order(cohort_stat$cohort, cohort_stat$statistic), , drop = FALSE]
  write.csv(cohort_stat, file.path(out_dir, "driver_stability_by_cohort_statistic.csv"), row.names = FALSE)
} else {
  cohort_stat <- data.frame()
}

if (nrow(drivers)) {
  gene_key <- interaction(drivers$cohort, drivers$sample, drivers$pathway, drivers$statistic, drivers$gene, drop = TRUE, sep = "\r")
  ...
  gene_stab <- gene_stab[order(gene_stab$cohort, gene_stab$sample, gene_stab$pathway,
...
md <- c(
  "# PASSAGE Driver-Gene Stability",
  "",
  paste0("- summary rows: ", nrow(summ)),
  paste0("- driver bootstrap rows: ", nrow(drivers)),
  paste0("- null rows: ", nrow(nulls)),
  "",
  "## Cohort/Statistic Summary",
  "",
  paste(c("cohort", "statistic", "n", "median_max_freq", "median_jaccard", "median_n_freq_ge_050", "frac_null_p_maxfreq_le_020", "frac_null_p_jaccard_le_020"), collapse = " | "),
  paste(rep("---", 8), collapse = " | ")
)
if (nrow(cohort_stat)) {
  for (ii in seq_len(nrow(cohort_stat))) {
    r <- cohort_stat[ii, ]
    md <- c(md, paste(c(r$cohort, r$statistic, r$n_pathway_samples,
